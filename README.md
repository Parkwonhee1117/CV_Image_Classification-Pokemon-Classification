...
1. **Transfer Learning 효과**
   ...
   * Pretrained 모델(Exp1, Exp2, Exp3)이 Random 초기화(Exp4)보다 높은 성능을 보였다.

2. **Fine-tuning 범위**

   * 전체 레이어를 학습한 경우(Exp2)가 FC만 학습한 경우(Exp1)보다 성능이 향상되었다.

3. **모델 구조 비교**

   * EfficientNet(Exp3)이 ResNet50과 비교하여 비슷하거나 더 높은 성능을 보였다.
